...
const callbackPage = (status: "success" | "error", data: { token?: string; message?: string }) => {
  ...
  responseHeaders.set(
    ...
    `default-src 'none'; script-src 'nonce-${nonce}'; frame-ancestors 'none'; base-uri 'none'; form-action 'none'`,
  );
  const message = JSON.stringify(
    `authorization:github:${status}:${JSON.stringify(data)}`,
  ).replaceAll("<", "\\u003c");
  return new Response(
    `<!doctype html><html lang="ja"><meta charset="utf-8"><title>CMS 認証</title>
...
const authorize = async (url: URL, env: Env) => {
  if (
    ...
    url.searchParams.get("site_id") !== "proshunsuke.github.io"
  ) {
...
const callback = async (request: Request, url: URL, env: Env) => {
  ...
  if (!state || state !== storedState || !(await validState(state, env.GITHUB_OAUTH_SECRET))) {
    return callbackPage("error", {
      message: "ログインの確認に失敗しました。管理画面からログインし直してください。",
    });
  }
