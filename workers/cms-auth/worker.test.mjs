...
const begin = async () => {
  const response = await worker.fetch(new Request(`${origin}/auth?provider=github&site_id=proshunsuke.github.io&scope=repo`), env);
  const location = new URL(response.headers.get('Location'));
  return { response, location, state: location.searchParams.get('state'), cookie: response.headers.get('Set-Cookie').split(';')[0] };
};
const finish = (state, cookie, suffix = '&code=test-code') => worker.fetch(new Request(`${origin}/callback?state=${state}${suffix}`, { headers: { Cookie: cookie } }), env);
...
test('rejects unknown provider, site, origin, method, and missing configuration', async () => {
  for (const query of ['provider=gitlab&site_id=proshunsuke.github.io', 'provider=github&site_id=evil.example', 'provider=github']) {
...
test('missing, mismatched, and forged state never exchanges a code', async (t) => {
  const mocked = t.mock.method(globalThis, 'fetch', () => { throw new Error('must not call GitHub'); });
  const { state, cookie } = await begin();
  const forged = `f${state.slice(1)}` === state ? `e${state.slice(1)}` : `f${state.slice(1)}`;
  for (const [value, saved] of [[state, ''], ['invalid', cookie], [forged, `__Host-cms-oauth=${forged}`]]) {
...
test('successful callback accepts messages only from the CMS opener', async (t) => {
  ...
  t.mock.method(globalThis, 'fetch', async (url, options) => {
    ...
    return Response.json(url.includes('access_token') ? { access_token: 'test-token', refresh_token: 'never-expose-refresh' } : { login: 'proshunsuke' });
  });
...
test('denied authorization, wrong user, and upstream failures never expose a token', async (t) => {
  const { state, cookie } = await begin();
  assert.match(await (await finish(state, cookie, '&error=access_denied')).text(), /authorization:github:error:/);
  const mock = t.mock.method(globalThis, 'fetch', async (url) => Response.json(url.includes('access_token') ? { access_token: 'private-test-token' } : { login: 'someone-else' }));
  ...
  mock.mock.mockImplementation(async () => { throw new Error('upstream-private-details'); });
