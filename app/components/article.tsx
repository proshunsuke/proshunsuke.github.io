...
export const Article = ({ title, html, headings, category }: ArticleProps) => (
  <div className="page-width py-10 sm:py-16">
    <nav
      aria-label="パンくず"
      className="mb-10 flex flex-wrap gap-2 text-sm text-muted"
    >
      <Link to="/">ホーム</Link>
    ...
    <header className="max-w-4xl border-b border-line pb-10">
      ...
      <h1 className="mt-4 text-3xl leading-snug font-bold tracking-tight sm:text-4xl">
        {title}
      </h1>
    </header>
            ...
            >
              {headings.map(({ id, label }) => (
                <a
                  key={id}
                  href={`#${id}`}
                  className="leading-relaxed hover:text-accent"
                >
                  {label}
                </a>
              ))}
