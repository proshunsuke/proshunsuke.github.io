...
CIのE2Eジョブは、ブラウザとOS依存ライブラリが入ったPlaywright公式Dockerイメージで実行し、毎回のブラウザ・OS依存ライブラリのインストールを省きます。イメージのバージョンはビルドジョブで `package-lock.json` から取得するため、npm側のPlaywrightと一致します。Node.jsはコンテナ内でもmiseで管理します。
